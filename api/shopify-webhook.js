...
export default async function handler(req, res) {
  ...
  // Shopify Flow sends the raw GraphQL response body
  const tableData = req.body?.data?.shopifyqlQuery?.tableData;
  if (!tableData) return res.status(400).json({ error: 'Missing tableData', received: req.body });
